...
/**
 * 地图标记物实体类 Created by WQ on 2017/8/30.
 */
...
public class ImageMark {
	...
	public static ImageMark createTestMark(double lat, double lng, Bitmap markeBitmap) {
		...
		// return createMark(lat, lng, markeBitmap, -markeBitmap.getWidth() *
		// (31f / 87f), -markeBitmap.getHeight());
		return createMark(lat, lng, markeBitmap, -markeBitmap.getWidth() * (1f / 2f),
				-markeBitmap.getHeight() * (1f / 2f));
	}
	...
	public String toString() {
		return "ImageMark{" + "lat=" + lat + ", lng=" + lng + ", xOffect=" + xOffect + ", yOffect=" + yOffect + '}';
	}
}
